...
export async function GET(req: NextRequest) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      (async () => {
        try {
          // Ensure env is present once the route is actually used
          getSupabase();
          ...
          if (isCountryIndex) {
            ...
            send({
              phase: "fetch",
              message: `Fetching competitions index: ${idxUrl}`,
              progress: 0.02,
            });
            ...
            for (let i = 0; i < comps.length; i++) {
              const comp = comps[i];
              const sliceStart = 0.05 + (0.9 - 0.05) * (i / comps.length);
              const sliceEnd = 0.05 + (0.9 - 0.05) * ((i + 1) / comps.length);

              const { clubsFound, playersSaved, clubsSkipped } =
          ...
          if (isCompStartseite || isCompCode) {
            ...
            const competitionCode =
              norm.match(/\/wettbewerb\/([^/?#]+)/i)?.[1] ?? null;
          ...
          throw new Error(
            "Unsupported path format. Pass a country index or competition URL."
          );
        } catch (err: any) {
...
async function scrapeOneCompetition(args: {
  ...
  const { startseitePath, competitionCode, season, send, sliceStart, sliceEnd } =
    args;

  const compUrl = withSearch(new URL(startseitePath, BASE), {
    saison_id: season,
  });
  send({
    phase: "fetch",
  ...
  for (const club of clubs) {
    ...
    const pBefore = lerp(
      sliceStart,
      sliceEnd,
      0.1 + 0.8 * (doneClubs / clubs.length)
    );
    send({
      phase: "fetch",
    ...
    try {
      const rosterHtml = await fetchHtmlWithRetry(
        new URL(club.kaderPath, BASE).toString()
      );
...
function extractCompetitionsFromCountry(html: string): Array<{
  ...
  $("table.items > tbody > tr").each((_, tr) => {
    ...
    const nameA = $tr
      .find("td.hauptlink table.inline-table a[href*='/startseite/wettbewerb/']")
      .last();
...
function extractClubsFromCompetitionStartseite(
  ...
  $("table.items > tbody > tr").each((_, tr) => {
    ...
    let kaderPath =
      $tr
        .find("a[href*='/kader/verein/']")
        .first()
        .attr("href") || profilePath.replace("/startseite/verein/", "/kader/verein/");
  ...
  if (!clubs.length) {
    $(".keys span").each((_, sp) => {
      const anchorHtml = $(sp).text();
      const $a = cheerio
        .load(anchorHtml)("a[href*='/startseite/verein/']")
        .first();
...
function ensureSeasonPlusInKaderPath(href: string, season: string) {
  ...
  if (!/\/saison_id\//.test(p)) {
    p = p.replace(
      /\/kader\/verein\/(\d+)/,
      `/kader/verein/$1/saison_id/${season}`
    );
  } else {
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `HTTP ${res.status} for ${url} — ${txt.slice(0, 120)}`
        );
      }
      return res.text();
    } catch (err) {
...
function parseRosterPlayers(html: string): ParsedPlayer[] {
  ...
  $("table.items > tbody > tr").each((_, tr) => {
    ...
    if (photo && photo.startsWith("/"))
      photo = new URL(photo, BASE).toString();

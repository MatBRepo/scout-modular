      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-stone-500 dark:text-neutral-400">
            ...
            tabeli <code className="text-[11px]">global_players</code> w
            Supabase.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <TabsList className="inline-flex gap-1 rounded-md bg-stone-100 p-1 text-[11px] dark:bg-neutral-900">
              <TabsTrigger
                value="lnp"
                className="px-2.5 py-1 text-[11px] leading-none"
              >
                LNP
              </TabsTrigger>
              <TabsTrigger
  ...
  return (
    <div className="space-y-4">
      <Card className="border-gray-200 bg-white/80 shadow-sm dark:border-neutral-800 dark:bg-neutral-950/80">
        <CardHeader className="flex flex-row items-center justify-between gap-3 border-b border-gray-100 pb-3 dark:border-neutral-800">
          <div>
            <div className="inline-flex h-6 items-center rounded-md bg-stone-100 px-2.5 text-[11px] font-medium uppercase tracking-wide text-stone-600 dark:bg-neutral-900 dark:text-neutral-200">
              Krok 1 · Wyszukiwanie w LNP
            </div>
            <div className="mt-2 text-sm font-semibold leading-none tracking-tight">
              Dodawanie zawodników z lokalnej bazy LNP
            </div>
            <p className="mt-1 text-[11px] text-stone-500 dark:text-neutral-400">
              Wyszukaj zawodników po nazwisku, klubie lub frazie. Wyniki możesz
              zapisać do lokalnej bazy w przeglądarce i wykorzystać później.
            </p>
          </div>
          <div className="flex flex-col items-end gap-1 text-[11px]">
            ...
            <Button
              ...
              onClick={importCSV}
            >
              <Upload className="h-3 w-3" /> Import z CSV
            </Button>
          </div>
        </CardHeader>
    ...
    <Card className="border-gray-200 bg-white/80 dark:border-neutral-800 dark:bg-neutral-950/80">
      <CardHeader className="flex flex-row items-center justify-between gap-3 py-2.5">
        <div>
          ...
          {error && (
            <div className="mt-0.5 flex items-center gap-1 text-[11px] text-rose-600 dark:text-rose-300">
              <AlertTriangle className="h-3 w-3" />
              Błąd źródła LNP: {error}
            </div>
          )}
          ...
          <table className="w-full text-[13px]">
            <thead className="sticky top-0 z-10 bg-stone-100 text-[11px] font-medium text-stone-700 shadow-sm dark:bg-neutral-900 dark:text-neutral-300">
              <tr>
                ...
                <th className="p-2.5 text-left hidden sm:table-cell">
                  Pozycja
                </th>
                <th className="p-2.5 text-left hidden sm:table-cell">
                  Wiek
                </th>
                <th className="p-2.5 text-left hidden md:table-cell">
                  Źródło
                </th>
              ...
              {rows.map((r) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-2.5 text-sm text-stone-800 dark:text-neutral-200">
                      {r.club || "—"}
                    </td>
...
function TmScraperPanel() {
  ...
  const loadPlayers = async (opts?: { page?: number }) => {
    ...
    setSidebarClub(null);
    setCollapsedClubs({}); // reset accordion
    ...
    try {
      ...
      const list: TmPlayerRow[] = rowsRaw.map((row: any) => ({
        ...
        number:
          row.number === undefined || row.number === null
            ? null
            : String(row.number),
        ...
        nationalities: Array.isArray(row.nationalities)
          ? row.nationalities.map((n: any) => String(n))
          : [],
        height_cm:
      ...
      const msg = `Załadowano stronę ${targetPage} (${list.length} rekordów) z ${
        count ?? "?"
      } pasujących w tm_flat_competition_players`;
      setLastMsg(msg);
      toast.success(msg);
    } catch (e: any) {
      console.error(e);
      const msg =
        e?.message ||
        "Nie udało się pobrać listy zawodników z tm_flat_competition_players";
      setErrorMsg(msg);
      setRows([]);
  ...
  const totalPages =
    totalRows && totalRows > 0
      ? Math.ceil(totalRows / TM_PAGE_SIZE)
      : null;
  ...
  const selectAllVisible = () => {
    setSelectedIds(filtered.map((p) => p.tm_player_id));
  };
  ...
  const sidebarPlayers = useMemo(() => {
    const base = sidebarClub
      ? filtered.filter(
          (p) => (p.club_name || "Bez klubu") === sidebarClub
        )
      : sortedPlayers;
  ...
  const saveSinglePlayer = async (p: TmPlayerRow) => {
    ...
    try {
      ...
      console.error(e);
      toast.error(
        `Błąd zapisu zawodnika: ${e?.message || "nieznany błąd"}`
      );
    } finally {
      setSaving(false);
    }
  ...
  const deleteSelected = () => {
    ...
    setRows((prev) =>
      prev.filter((p) => !selectedIds.includes(p.tm_player_id))
    );
    if (
      selectedPlayer &&
      selectedIds.includes(selectedPlayer.tm_player_id)
    ) {
      setSelectedPlayer(null);
      setPlayerDetails(null);
      setDetailsError(null);
  ...
  const loadPlayerDetails = async (player: TmPlayerRow) => {
    ...
    try {
      const url =
        `${TM_DETAILS_ENDPOINT}?path=` +
        encodeURIComponent(player.player_path);
      ...
      console.log("[TM DETAILS RAW]", { url, status: res.status, text });
      ...
      console.log("[TM DETAILS PARSED]", json);
  ...
  const renderDetailsRow = (
    rowPlayer: TmPlayerRow,
    isRowDetailsLoading: boolean
  ) => {
    if (
      !selectedPlayer ||
      selectedPlayer.tm_player_id !== rowPlayer.tm_player_id
    ) {
      return null;
    }
                ...
                <div className="space-y-1">
                  ...
                  {(facts?.birthPlace || facts?.citizenship) && (
                    <div className="text-[11px] text-muted-foreground">
                      ...
                      {facts?.birthPlace && facts?.citizenship && " • "}
                      {facts?.citizenship && (
                        <>Obywatelstwo: {facts.citizenship}</>
                      )}
                    </div>
                  )}
                </div>
            ...
            {playerDetails && (
              <Tabs defaultValue="currentSeason" className="mt-1 w-full text-xs">
                <TabsList className="mb-2 flex flex-wrap gap-1 bg-emerald-100/60 p-1 text-[11px] text-muted-foreground/70 dark:bg-emerald-950/40">
                  <TabsTrigger value="currentSeason">
                    Current season
                  </TabsTrigger>
                  <TabsTrigger value="allSeasons">
                    All seasons
                  </TabsTrigger>
                  <TabsTrigger value="byClub">
                    Stats by club
                  </TabsTrigger>
                  <TabsTrigger value="injuries">
                    Injury history
                  </TabsTrigger>
                  <TabsTrigger value="marketValue">
                    Market value
                  </TabsTrigger>
                </TabsList>
  ...
  return (
    <div className="space-y-5">
      {/* Nagłówek TM – w stylu kroków jak w PlayerEditorPage */}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                ...
                onClick={() =>
                  void loadPlayers({
                    page:
                      totalPages && page < totalPages ? page + 1 : page,
                  })
                }
        ...
        <Card className="border-gray-200 bg-white/80 p-3 sm:p-4 dark:border-neutral-800 dark:bg-neutral-950/80">
          <CardTitle className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-2">
              ...
              <span>
                Lista zawodników
                {sidebarClub ? ` – ${sidebarClub}` : " (alfabetycznie)"}
              </span>
            </div>
      ...
      <Card className="p-0 border-gray-200 bg-stone-50 dark:border-neutral-800 dark:bg-neutral-950">
        <div className="w-full overflow-x-auto">
          <table className="w-full text-[13px]">
            <thead className="sticky top-0 z-10 bg-stone-100 text-[11px] font-medium text-stone-700 shadow-sm dark:bg-neutral-900 dark:text-neutral-300">
              <tr>
                <th className="w-[32px] px-2 py-2.5">
                  <div className="flex justify-center">
                    <Checkbox
                      ...
                      onCheckedChange={(v) =>
                        v ? selectAllVisible() : clearSelection()
                      }
                ...
                <th className="p-2.5 text-left hidden lg:table-cell">
                  Kontrakt do
                </th>
                <th className="p-2.5 text-right">Akcje</th>
              </tr>
                ...
                groupedByClub.map((group) => {
                  ...
                  return (
                    <React.Fragment key={`club-${keyName}`}>
                      {/* CLUB HEADER – as accordion toggle */}
                      ...
                      {/* CLUB PLAYERS – hidden when collapsed */}
                      {!isCollapsed &&
                        group.players.map((p, idx) => {
                          const tmHref = openTmUrl(
                            p.player_path || undefined
                          );

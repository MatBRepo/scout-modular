...
function SavePill({ state }: { state: "idle" | "saving" | "saved" }) {
  // Match shadcn Button height/shape (h-10, px-3, rounded)
  const base =
...
export function ObservationEditor({
  ...
  return (
    <div className="w-full">
      ...
      <Toolbar
        title={
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-lg font-semibold mb-2">Mecz: {o.match || "—"}</span>
            <span className="text-xs text-dark">{headerMeta}</span>
          </div>
        }
        right={
          <div className="mt-2 flex flex-wrap items-center gap-2 sm:mt-0">
            {saveState !== "idle" ? <SavePill state={saveState} /> : <SavePill state="idle" />}
            <Button
              variant="outline"
              className="h-10 border-gray-300 focus-visible:ring focus-visible:ring-indigo-500/60 dark:border-neutral-700"
              onClick={onClose}
            >
              Anuluj
            </Button>
      ...
      {/* No parent Card wrapper; just separated full-width boxes */}
      <div className="mt-4 space-y-6">
        ...
        >
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              <Label className="text-sm">Data raportu</Label>
              <Input
                type="date"
                value={o.reportDate ?? ""}
            ...
            <div>
              <Label className="text-sm">Mecz jakiej ligi/turnieju</Label>
              <Input
                value={o.competition ?? ""}
            ...
            <div>
              <Label className="text-sm">Poziom rywala</Label>
              <select
                value={o.opponent ?? ""}
                onChange={(e) => set("opponent", e.target.value as OpponentLevel)}
            ...
            <div>
              <Label className="text-sm">Warunki obserwacji</Label>
              <select
                value={o.conditions ?? "live"}
                onChange={(e) => set("conditions", e.target.value as Mode)}
                className="mt-1 w-full rounded border border-gray-300 bg-white p-2 text-sm dark:border-neutral-700 dark:bg-neutral-950"
              >
                {CONDITIONS.map((x) => (
                  <option key={x} value={x}>
            ...
            <div className="md:col-span-2">
              <Label className="text-sm">Uwagi kontekstowe</Label>
              <Textarea
                value={o.contextNote ?? ""}
        ...
        >
          ...
          <p className="text-sm text-dark dark:text-neutral-400">

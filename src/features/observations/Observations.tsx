...
type ObsPlayer = {
  ...
  // ⬇️ KEY: same as in ObservationEditor
...
const COL_NAMES: Record<ColKey, string> = {
  select: "#",
  match: "Match",
  date: "Date",
  time: "Time",
...
export default function ObservationsFeature({
  ...
  useEffect(() => {
    ...
    // if filters closed, ensure unlocked
    if (isMobile && !filtersOpen) {
      unlockBodyScroll();
    }
  ...
  // --- NEW: transient inline status note (left, although the status column is now the league) ---
      ...
      const av = (() => {
        switch (sortKey) {
          ...
          case "mode":
            return a.mode ?? "live";
          case "status":
            // sort key "status" is now used to sort by league (competition)
  ...
  function addNew() {
    setEditing({
      ...
      bucket: "active",
      mode: "live",
      competition: null, // new observation without a league
      note: "",
      voiceUrl: null,
      players: [],
    });
  ...
  // (left - in case you have the status column again)
        ...
        {moreOpen &&
          (isMobile ? (
            ...
            >
              ...
              {/* Columns opens a separate popover anchored to the dots */}
              <button
                      ...
                      {visibleCols.actions && (
                        <td className={`${cellPad} text-right align-center pr-4`}>
                          <div className="inline-flex items-center gap-2">
                            ...
                            {/* Trash / Restore with Yes / No confirmation */}

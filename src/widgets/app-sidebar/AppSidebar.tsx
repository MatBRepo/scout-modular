...
export default function AppSidebar({
  ...
  const InnerNav = () => (
    <nav className="space-y-1 text-sm" onClick={() => onClose?.()}>
      ...
      {/* PLAYERS (Scout / Scout-Agent / Admin) */}
    ...
    <div className="flex h-full flex-col">
      ...
      <div ref={accountRef} className="relative">
        <button
          ...
          className="flex w-full items-center justify-between rounded-md px-3 py-2 text-sm text-gray-800 transition hover:bg-gray-50 dark:text-neutral-100 dark:hover:bg-neutral-900"
          aria-haspopup="menu"
          aria-expanded={accountOpen}
        >
          <span className="flex min-w-0 items-center gap-2">
            <span className="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-[10px] font-semibold text-indigo-800 ring-1 ring-black/5 dark:bg-indigo-900/40 dark:text-indigo-200">S4</span>
            <span className="truncate">Konto</span>
            {role === "scout" && (
              <span className={`ml-2 inline-flex min-w-0 items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold ring-1 ${rankClass(rank)} max-w-[6.5rem]`}>
